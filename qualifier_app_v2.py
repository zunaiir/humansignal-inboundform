...
with col2:
    ...
    if submitted:
        if (
            ...
            assigned_ae = result.get("assigned_ae", "")
            assigned_ae_email = result.get("assigned_ae_email", "")

            st.markdown('<div class="internal-card">', unsafe_allow_html=True)
            st.markdown("### Internal Qualification Result")

            if decision == "enterprise_ae":
                st.markdown(
                    '<span class="pill pill-green">Enterprise AE</span>',
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    '<span class="pill pill-yellow">Manual Review</span>',
                    unsafe_allow_html=True
                )

            st.markdown(
                f"**Decision:** {'Qualified for Enterprise AE follow-up' if decision == 'enterprise_ae' else 'Send to manual review'}  \n"
                f"**Confidence:** {result.get('confidence', '—')}  \n"
                f"**Assigned AE:** {assigned_ae or 'Unassigned'}"
            )

            if assigned_ae_email:
                st.markdown(f"<div class='small-muted'>AE Email: {assigned_ae_email}</div>", unsafe_allow_html=True)

            st.markdown('<div class="section-title">Reasoning</div>', unsafe_allow_html=True)
            st.markdown(
                f"<div class='reason-box'>{llm.get('reasoning', 'No reasoning returned.')}</div>",
                unsafe_allow_html=True
            )

            st.markdown('<div class="section-title">Lead Details</div>', unsafe_allow_html=True)
            st.markdown(
                f"**Contact:** {lead.get('full_name', '')}  \n"
                f"**Company:** {lead.get('company', '')}  \n"
                f"**Email:** {lead.get('company_email', '')}  \n"
                f"**Using OSS:** {lead.get('using_label_studio_oss', '')}  \n"
                f"**Lead ID:** {result.get('lead_id', '')}"
            )

            st.markdown('<div class="section-title">Qualification Summary</div>', unsafe_allow_html=True)
            st.markdown(
                f"- Industry: **{llm.get('industry', 'Unknown')}**  \n"
                f"- Project Scope: **{llm.get('project_scope', 'Unknown')}**  \n"
                f"- Use Case Complexity: **{llm.get('use_case_complexity', 'Unknown')}**  \n"
                f"- Regulated Data: **{llm.get('regulated_data_flag', False)}**  \n"
                f"- Urgency: **{llm.get('urgency', 'Unknown')}**  \n"
                f"- Buyer Seriousness: **{llm.get('buyer_seriousness', 'Unknown')}**  \n"
                f"- Enterprise Score: **{llm.get('enterprise_score', 0)}**"
            )
            ...
            if signals:
                ...
                st.markdown("- No explicit signals returned.")

            st.markdown('<div class="section-title">Internal Summary</div>', unsafe_allow_html=True)
            st.markdown(
                f"<div class='reason-box'>{llm.get('draft_internal_summary', 'No summary returned.')}</div>",
                unsafe_allow_html=True
            )

            st.markdown('<div class="section-title">System Actions</div>', unsafe_allow_html=True)
            if decision == "enterprise_ae":
                st.success(
                    f"Lead routed to {assigned_ae}. "
                    f"Google Sheets logging and AE email notification were triggered."
                )
            else:
                st.warning(
                    "Lead sent to manual review. "
                    "Google Sheets logging and manual review email notification were triggered."
                )

            if result.get("website"):
                st.markdown(f"**Inferred Website:** {result.get('website')}")
